        ...
        {projects.map(item => (
            <Box width={"100%"} key={item.id} >
                  <Box  width={"100%"} textAlign={"center"} mt={4} onClick={() => router.push(`${item.proAdress}`)} sx={{borderRadius: "20px"}}>
                      <Image1 src={item.proImage.url} alt={item.proName} priority={true} width={500} height={500}  className="project_slider_img" style={{borderRadius: "20px",cursor: "pointer", boxShadow:"6px 10px 40px 0px #dcdcdc"}} />
